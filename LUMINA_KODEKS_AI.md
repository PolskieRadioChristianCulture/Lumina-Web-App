...
2. **Logika Przyczynowo-Skutkowa w UI/UX (Zero Dead-Ends & Zero Confusions):**
   ...
   - Pasek akcji na profilach (`.head-actions`) zawsze zachowuje ergonomiczny, wyśrodkowany układ poziomy (`flex-direction: row; justify-content: center;`).
   - **Żelazna Doktryna Mobilna Profili — Wyłącznie Ikony (Mobile Icon-Only Doctrine):** Na profilach użytkowników w widoku mobilnym (`max-width: 768px`) wszystkie przyciski akcji nagłówka (`.head-actions > *`, `.btn-action-*`), zakładki nawigacji (`.profile-nav-tabs .tab-btn`) oraz przycisk powrotu w navbarze (`.btn-nav-back`) **OGRANICZAJĄ SIĘ BEZWZGLĘDNIE I WYŁĄCZNIE DO CZYSTYCH IKON**. Obowiązuje całkowity zakaz renderowania etykiet tekstowych, liczników czy klocków 2x2 z `min-width: 130px`. Przyciski akcji stają się okrągłym dockiem `44x44px` (`border-radius: 50%`, Apple HIG / Google Material touch target), a zakładki nawigacji to `44x44px` (`border-radius: 14px`). Gwarantuje to zero ucinania tekstu, zero poziomego przewijania i luksusowy, aplikacyjny minimalizm.
