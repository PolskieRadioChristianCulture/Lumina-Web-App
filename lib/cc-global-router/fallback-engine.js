 ...
 * Dyrektywy 5, 6, 9, 32, 33: Wave 2 Disabled Filter, Format Fallback vs Language Fallback
...
export function resolveWithFallback(requestedLocale, contentMaster, options = {}) {
    ...
    for (let i = 0; i < chain.length; i++) {
        ...
        // Dyrektywa 5: Sprawdzenie czy dany język jest aktywny w Locale Registry (Wave 1 vs Wave 2)
        const locDef = getLocale(candidate);
        if (!locDef || !locDef.enabled) {
            if (i === 0) {
                firstFailureReason = 'LOCALE_DISABLED';
            }
            continue; // Pomiń nieaktywny język (np. de, fr, it, uk)
        }
        ...
        if (!contentMaster) {
            ...
            return {
                ...
                fallbackReason: isFallback ? (firstFailureReason || `LOCALE_NOT_ENABLED (${requestedLocale})`) : null,
        ...
        if (availability.available) {
            ...
            if (isFallback) {
                if (firstFailureReason === 'LOCALE_DISABLED') {
                    finalReason = 'LOCALE_DISABLED';
                } else if (isFormatIssue) {
                    finalReason = `FORMAT_FALLBACK (${firstFailureReason})`;
                } else {
                    finalReason = `LANGUAGE_FALLBACK (${firstFailureReason || requestedLocale})`;
                }
            }
            ...
            if (i === 0) {
                ...
                if (availability.reason && availability.reason.includes('AUDIO')) {
                    isFormatIssue = true;
                }
            }
        }
    ...
    return {
        ...
        fallbackUsed: true,
        fallbackReason: isFormatIssue ? `FORMAT_FALLBACK (${firstFailureReason})` : (firstFailureReason || 'CONTENT_UNAVAILABLE'),
        contentVariantId: null,
        attemptedLocales
    };
}
