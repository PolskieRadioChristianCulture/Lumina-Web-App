...
export async function listContent(db, queryOptions = {}) {
    ...
    try {
        ...
        if (err.message && err.message.includes('requires an index')) {
            console.warn('[CONTENT_CORE] Indeks złożony jest w trakcie budowania w Firestore — fallback do zapytania bazowego.');
            let fallbackQ = db.collection(CC_COLLECTION_NAME);
            if (queryOptions.status) fallbackQ = fallbackQ.where('status', '==', queryOptions.status);
            const snap = await fallbackQ.limit(pageSize).get();
            const docs = snap.docs.map(d => d.data()).sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
            return {
                items: docs,
