...
import { CC_ANALYTICS_DICTIONARY, getMetricDefinition } from './cc-analytics-dictionary.js';

export { CC_ANALYTICS_DICTIONARY, getMetricDefinition };
...
export const ALLOWED_METRIC_TYPES = new Set([
    ...
    'PAGE_VIEWS',
    'SCROLL_DEPTH_AVG',
    // Faza 5: CC GEO & Language Router Metrics (Dyrektywa 60)
    'ROUTING_DECISION',
    'LANGUAGE_OVERRIDE',
    'FALLBACK_USED',
    'CONTENT_LOCALE_UNAVAILABLE',
    'GEO_HINT_USED',
    // Gate 5.6: Controlled Activation & Suggestions Metrics
...
/**
 * TELEMETRY FIELD ALLOWLIST (Gate 4.5 Dyrektywy 3, 4)
 * Obserwator może zapisywać wyłącznie jawnie dozwolone pola.
 * Wszystkie pozostałe pola są odrzucane domyślnie (DROP BY DEFAULT).
 */
export const TELEMETRY_FIELD_ALLOWLIST = new Set([
    'eventId',
    'contentId',
    'variantId',
    'publicationId',
...
/**
 * PII DENYLIST (Gate 4.5 Dyrektywy 5, 6)
 * Jawnie odrzucane pola danych osobowych i sekretów.
 * ZERO heurystyk na imionach autorów czy postaci biblijnych!
 */
export const PII_DENYLIST = new Set([
    'email',
    'userEmail',
    'phone',
    'userPhone',
...
/**
 * Polityka Retencji Danych Analitycznych (Gate 4.5 Dyrektywa 35)
 */
export const RETENTION_POLICY = Object.freeze({
    rawEventsRetentionDays: 90,
    snapshotsRetentionDays: 365,
    rollupsRetentionDays: 365 * 5, // 5 lat
    incidentsRetentionDays: 365 * 2 // 2 lata
});

/**
 * Priorytety Odpytywania i Cost Guard (Gate 4.5 Dyrektywy 36, 37)
 */
export const POLLING_PRIORITIES = Object.freeze({
    RECENT_ACTIVE: {
        maxAgeHours: 48,
        intervalMinutes: 30,
        label: 'RECENT_ACTIVE (Emisja <= 48h)'
    },
    OLDER: {
        maxAgeHours: 24 * 30, // 30 dni
        intervalMinutes: 360, // 6h
        label: 'OLDER (Emisja 2-30 dni)'
    },
    ARCHIVED: {
        maxAgeHours: Infinity,
        intervalMinutes: 1440, // 24h
        label: 'ARCHIVED (Emisja > 30 dni)'
    }
});

/**
 * Zwraca priorytet odpytywania na podstawie wieku publikacji
 * @param {string|Date} publishedAt
 */
export function getPollingPriority(publishedAt) {
    if (!publishedAt) return POLLING_PRIORITIES.RECENT_ACTIVE;
    const ageHours = (Date.now() - new Date(publishedAt).getTime()) / (1000 * 3600);
    if (ageHours <= 48) return POLLING_PRIORITIES.RECENT_ACTIVE;
    if (ageHours <= 24 * 30) return POLLING_PRIORITIES.OLDER;
    return POLLING_PRIORITIES.ARCHIVED;
}
...
export function createAnalyticsEvent(params) {
    ...
    // Gate 4.5 Dyrektywy 3-6: Telemetry Allowlist + PII Denylist (Zero heurystyk na imionach autorów!)
    // Blokada jawnych pól PII
    for (const piiKey of PII_DENYLIST) {
        if (params[piiKey] !== undefined) {
            delete params[piiKey];
        }
    }

    // IP Handling (Dyrektywa 6): Edge processing -> country/coarse region -> DROP IP
    let region = params.region || params.regionCode || params.countryCode || null;
    if (params.ipAddress || params.ip) {
        // Jeśli podano IP bez regionu, przypisujemy zgrubny kod i niszczymy surowe IP
        if (!region) {
    ...
    const observationMode = params.observationMode || 'PERIODIC';
    ...
    return {
        ...
        countryCode: params.countryCode || (region && region.length === 2 ? region : null),
 ...
 */
export function compareLanguages(input = {}, minThreshold = 100, options = {}) {
    const supportedLangs = ['pl', 'en', 'es', 'pt-BR'];
    const languages = {};
    const threshold = options.minSampleSize ?? minThreshold ?? 100;
    ...
    for (const lang of supportedLangs) {
        ...
        const isSufficient = count >= threshold;
