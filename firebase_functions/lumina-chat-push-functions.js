...
exports.onLuminaPostCreated = onDocumentCreated(
    ...
    async (event) => {
        ...
        const result = await sendToTokens([...followerTokens], {
            ...
            icon: authorAvatar,
            image: post.imageUrl || undefined,
            url: `https://polskieradio.cc/lumina-tablica.html?postId=${postId}&author=${authorSlug}`,
...
// ══════════════════════════════════════════════════════════════════════════
// 3b) PROŚBY O ROZMOWĘ — PUSH DO ODBIORCY PIERWSZEGO KONTAKTU
// ══════════════════════════════════════════════════════════════════════════
exports.onMessageRequestCreated = onDocumentCreated(
    { document: 'lumina_message_requests/{requestId}', region: REGION },
    async (event) => {
        const request = event.data?.data();
        if (!request?.receiverId || !request?.senderId || request.receiverId === request.senderId) return;
        const tokens = await getTokensForUser(request.receiverId);
        if (!tokens.length) return;
        const senderName = request.senderName || 'Członek społeczności LUMINA';
        const result = await sendToTokens(tokens, {
            title: `💌 ${senderName} chce rozpocząć rozmowę`,
            body: (request.previewText || 'Otwórz prośbę o rozmowę.').slice(0, 140),
            icon: request.senderAvatar || 'https://polskieradio.cc/lumina_icon.jpg',
            url: 'https://polskieradio.cc/lumina.html?openMessages=private',
            tag: `lumina-request-${request.senderId}`,
            requireInteraction: true,
            data: { type: 'direct_message_request', senderId: request.senderId, requestId: event.params.requestId || '' },
            actions: [{ action: 'open', title: 'Zobacz prośbę' }, { action: 'reply', title: 'Otwórz rozmowy' }]
        });
        logger.info(`[Push request] ${request.senderId} → ${request.receiverId}: wysłano ${result.sent}, błędy ${result.failed}`);
    }
);
...
// ══════════════════════════════════════════════════════════════════════════
// 6) NIEDZIELNY APEL MISYJNY CHRISTIAN CULTURE — W KAŻDĄ NIEDZIELĘ O 09:00
// ══════════════════════════════════════════════════════════════════════════
exports.scheduledSundayMissionAppealPush = onSchedule(
    { schedule: '0 9 * * 0', timeZone: 'Europe/Warsaw', region: REGION },
    async (event) => {
        logger.info('[Push Niedziela 09:00] Uruchomienie Niedzielnego Apelu Misyjnego Christian Culture...');

        const allTokens = await getAllActiveDeviceTokens([]);
        if (!allTokens.length) {
            logger.info('[Push Niedziela 09:00] Brak zarejestrowanych urządzeń.');
            return;
        }

        const result = await sendToTokens(allTokens, {
            title: '🌿 APEL MISYJNY CHRISTIAN CULTURE • Niedziela',
            body: 'Razem budujemy Christian Culture 🌿🤍 Dzisiejsza niedziela to czas wdzięczności za wspólnotę. Wspieraj Bożą misję!',
            icon: 'https://polskieradio.cc/lumina_icon.jpg',
            image: 'https://polskieradio.cc/apel_misyjny_cc.webp?v=20260920_blik537137043',
            url: 'https://polskieradio.cc/lumina-tablica.html?post=post_sunday_mission_appeal#sundayAppeal',
            tag: 'lumina-sunday-mission-appeal',
            requireInteraction: true,
            data: {
                type: 'sunday_appeal',
                url: 'https://polskieradio.cc/lumina-tablica.html?post=post_sunday_mission_appeal#sundayAppeal'
        ...
        logger.info(`[Push Niedziela 09:00] Wysłano ${result.sent} powiadomień apelu misyjnego, błędy: ${result.failed}`);
    }
);

// ══════════════════════════════════════════════════════════════════════════
// 7) CODZIENNA MISJA — POWIADOMIENIE WIECZORNE CODZIENNIE O 19:00
// ══════════════════════════════════════════════════════════════════════════
exports.scheduledDailyMissionEveningPush = onSchedule(
    { schedule: '0 19 * * *', timeZone: 'Europe/Warsaw', region: REGION },
    async (event) => {
        logger.info('[Push 19:00] Uruchomienie wieczornego powiadomienia Codzienna Misja...');

        const allTokens = await getAllActiveDeviceTokens([]);
        if (!allTokens.length) {
            logger.info('[Push 19:00] Brak zarejestrowanych urządzeń.');
            return;
        }

        const result = await sendToTokens(allTokens, {
            title: '🕊️ Codzienna Misja • Christian Culture',
            ...
            actions: [
                ...
                { action: 'blik', title: '📱 Misyjny BLIK (537 137 043)' }
            ]
        });

        logger.info(`[Push 19:00] Wysłano ${result.sent} powiadomień Codziennej Misji, błędy: ${result.failed}`);
    }
);
